  ...
  return (
    <>
      <style>{`
        ...
        @media (max-width: 639px) { .ex-stat-num { font-size: 1.6rem; } .ex-img-card { padding: 18px; border-radius: 20px; } .ex-stat-card { padding: 16px; } }
      `}</style>

      <section ref={sectionRef} className="ex-section" style={{ background: '#fff', position: 'relative', overflow: 'hidden', padding: isMobile ? '52px 16px' : isTablet ? '64px 24px' : '80px 28px' }}>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          ...
          <div className="ex-fade" style={{ textAlign: 'center', marginBottom: isMobile ? 36 : 52 }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '10px', padding: '8px 18px', borderRadius: '16px', background: '#fff', border: '1px solid rgba(255,255,255,0.6)', color: '#FF1F8E', fontWeight: 700, fontSize: '10px', letterSpacing: '0.3em', textTransform: 'uppercase', marginBottom: '1.2rem', boxShadow: '0 1px 6px rgba(0,0,0,0.05)' }}>
            ...
            <h2 style={{ fontSize: isMobile ? '2rem' : 'clamp(2.5rem, 5vw, 4rem)', fontWeight: 900, color: '#0a0a0a', letterSpacing: '-0.04em', lineHeight: 1.05, marginBottom: 12 }}>
              Built on{' '}
            ...
            <p style={{ fontSize: isMobile ? 15 : 18, color: '#94a3b8', fontWeight: 500, lineHeight: 1.7, maxWidth: 440, margin: '0 auto' }}>
              We don&apos;t just ship code — we become an extension of your team and deliver products that actually move the needle.
            </p>
          </div>

          {/* Two-col layout */}
          <div style={{ display: 'grid', gridTemplateColumns: isDesktop ? 'minmax(0,1fr) minmax(0,1fr)' : '1fr', gap: isMobile ? 20 : 28, alignItems: 'start' }}>

            {/* LEFT */}
            <div className="ex-img-card ex-fade" style={{ transitionDelay: '0.1s' }}>
              <Image src={dev} alt="Webstep Solutions Development" style={{ width: '100%', height: 'auto', borderRadius: 18, marginBottom: 22, display: 'block' }} />
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                <div>
                  <div style={{ display: 'flex', gap: 3, marginBottom: 3 }}>
                    {[1,2,3,4,5].map(s => <FaStar key={s} style={{ color: '#F59E0B', fontSize: 16 }} />)}
                  </div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: '#0a0a0a' }}>5.0 · 100+ reviews</div>
                </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {reviews.map((r, i) => (
                  <div key={i} className="ex-review-row">
                    <div style={{ width: 34, height: 34, borderRadius: '50%', background: r.avatarBg, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 900, color: r.avatarColor }}>
                      {r.name[0]}
                    </div>
                    <div>
            ...
            <div className="ex-fade" style={{ transitionDelay: '0.2s' }}>
              <p style={{ fontSize: 10, fontWeight: 800, letterSpacing: '0.3em', textTransform: 'uppercase', color: '#93989e', marginBottom: 10 }}>Why Clients Choose Webstep</p>
              <h3 style={{ fontSize: isMobile ? '1.5rem' : 'clamp(1.6rem,3vw,2.2rem)', fontWeight: 900, color: '#0a0a0a', letterSpacing: '-0.03em', lineHeight: 1.1, marginBottom: 28 }}>
                Numbers that{' '}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: isMobile ? 10 : 12, marginBottom: 28 }}>
                {stats.map((item, idx) => (
              ...
              <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', flexWrap: 'wrap', gap: 10 }}>
                <Link href="/case-study" className="ex-btn-primary" style={{ flex: isMobile ? '1' : 'unset' }}>See Our Work</Link>
                <Link href="/customize-package" className="ex-btn-secondary" style={{ flex: isMobile ? '1' : 'unset' }}>Start a Project →</Link>
              </div>

              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 20, padding: isMobile ? '12px 14px' : '14px 18px', background: '#fafafa', border: '1px solid #f0f0f4', borderRadius: 14 }}>
